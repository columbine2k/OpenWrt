...
# ============================================================
# lede/diy.sh：编译前对 OpenWrt/LEDE 源码做定制
#
# 约定：
#   - 对上游文件的具体修改一律做成补丁放在 lede/patches/*.patch，
...
_patch_dir="$GITHUB_WORKSPACE/lede/patches"
...
cp -f $GITHUB_WORKSPACE/lede/rootfs/etc/init.d/transmission files/etc/init.d/transmission
chmod +x files/etc/init.d/transmission
cp -f $GITHUB_WORKSPACE/lede/rootfs/etc/uci-defaults/99-transmission-dirs files/etc/uci-defaults/99-transmission-dirs
